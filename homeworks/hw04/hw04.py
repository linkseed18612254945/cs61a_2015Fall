...
def quadratic(x, a, b, c):
    ...
    '0 to 10'
    """
    f = lambda n: a * n * n + b * n + c
    point = -b / (2 * a)
    if lower_bound(x) < point < upper_bound(x):
        return interval(min(f(point), f(lower_bound(x)), f(upper_bound(x))), max(f(point), f(lower_bound(x)), f(upper_bound(x))))
    else:
...
def close_enough(x, target):
    return math.fabs(x - target) <= 1e-5


def find_root(f, init):
    """
    Use Newton Method to find root of function
    :param f:
    :return:
    """
    x = init
    fx = f(init)
    while not close_enough(fx, 0):
        x, fx = newton_update(f, x)
    return x
