...
def city_distance(city1, city2):
    return (get_lat(city1) - get_lat(city2)) ** 2 + (get_lon(city1) - get_lon(city2)) ** 2
...
def closer_city(lat, lon, city1, city2):
    """ Returns the name of either city1 or city2, whichever is closest
        ...
        'Bucharest'
    """
    my_position = make_city('my_position', lat, lon)
    if city_distance(my_position, city1) > city_distance(my_position, city2):
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    for row_num in range(max_rows - 1, -1, -1):
        if board[row_num][column] == '-':
            board[row_num] = replace_elem(board[row_num], column, player)
            return row_num, board
    return -1, board
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        for col in range(max_cols):
            board_graph = board_graph + get_piece(board, row, col) + ' '
        board_graph = board_graph.rstrip()
        board_graph += '\n'
    print(board_graph.rstrip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    connected_piece = 0
    for piece in get_row(board, row):
        if piece == player:
            connected_piece += 1
        else:
            connected_piece = 0
        if connected_piece == num_connect:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    """
    connected_piece = 0
    for piece in get_col(board, col):
        if piece == player:
            connected_piece += 1
        else:
            connected_piece = 0
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    return check_win_row(board, max_cols, max_cols, num_connect, row, player) or \
           check_win_column(board, max_cols, max_cols, num_connect, col, player) or diagonal_win
